...
mode=WebRtcMode.SENDRECV,
rtc_configuration=RTC_CONFIGURATION,
media_stream_constraints={"video": {"width" : 1920, "height" : 1080}}, # 704 * 396 웹에 표현되는 크기
video_processor_factory = VideoProcessor,
async_processing=True,
)
...
if file_path is not None:
    ...
    canvas = np.zeros((704, 396,3), np.uint8)

    # Loop through the landmark data
    for i in range(df4.shape[0]):
        for j in range(int(df4.shape[1]/2)):
            x, y = df4.iloc[:,[2*j, 2*j +1]]
            x = x * 704  #denormalize
            y = y * 396
            print(x, y)
            if x >= 0 and x <= 704 and y >= 0 and y <= 396:
                # Plot a circle on the canvas using OpenCV's circle function
                cv2.circle(canvas, (int(x), int(y)), 2, (255, 0, 0), -1)   #canvas, 좌표, 반지름, 색깔, 테두리
            else: 
                cv2.circle(canvas, (int(704/2),int(396/2)), 2, (255, 0, 0), -1)
    # Show the heatmap
